...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-test-547-infrastructure \
     --values test-test-547-infrastructure/values.yaml \
     --values test-test-547-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/JeffreyRoss/test-547 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "test-test-547 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-test-547 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace test-test-547 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/JeffreyRoss/test-547 \
      --path test-test-547-deploy/src/main/resources \
      --revision $branch \
...
down() {
  if is_app_running; then
    ...
    echo "test-test-547 is not deployed"
  fi
}
